From now on, we will need a 2nd terminal open. See the plus symbol above the terminal window? Use this to add another shell terminal and then use the 2nd terminal to start and stop the webserver (-> Hint: `/apache/bin/httpd -X`).
